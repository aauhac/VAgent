...
**Audit status:** `PASS`  
**Audio id:** `00ae450b11f1`

## 기본 정보

- 파일: `C:\VocalAgent\목잡이.m4a`
- SHA: `00ae450b11f15f53af9e4ea58c4e52d4bdde098e292c5e6530675c775de0fac1`
- 길이: None
- Sample Rate: None
- Analysis status: OK
- Source: fresh
...
| 흉성/두성 음향 성향 | BALANCED | — | 흉성·두성 균형에 가까운 편 |
| 안정성 | STABLE | — | 안정성이 유지되는 편 |
| 중역 존재감 | MID | — | 중역 존재감이 중간 편 |
| 밝기 | LOW | — | 밝기가 어두운 편 |
| 음색의 공기감 | LOW | — | LOW |
...
접촉감은 단단한 편이지만, 성구 연결이 일부 구간에서만 안정적으로 이어지는, 발성 안정성이 유지되는 편인, 밝기가 어두운 편인, 숨 섞임이 적은 편인 발성으로 분석됐어요.
...
| 고민 | Focus | 첫 처방 | Protocol | Status |
|---|---|---|---|---|
| 내 음색이 마음에 들지 않아요 | EFFORT | 같은 음량에서 자음 시작을 조금 더 분명하게 하고 모음을 오래 눌러 붙이지 않은 채 짧… | EFFORT | PASS |
| 소리가 얇고 모기소리처럼 느껴져요 | REGISTER_CONNECTION | 얇게 느껴지지 않는 쪽으로, 얇게 느껴지는 구절에서 먼저 '우'처럼 편하게 이어지는 모… | REGISTER_CONNECTION | PASS |
| 소리가 답답하게 들려요 | EFFORT | 답답·어두운 인상이 덜한 쪽으로, 편한 중음의 짧은 구절에서 음량은 그대로 두고 자음 … | EFFORT | PASS |
| 콧소리처럼 들려요 | REGISTER_CONNECTION | 콧소리처럼 들리지 않는 쪽으로, 콧소리처럼 느껴지는 모음·음절 하나만 골라, 같은 음높… | REGISTER_CONNECTION | PASS |
| 숨이 많이 섞여요 | EFFORT | 숨 섞임이 과해지지 않는 쪽으로, 짧은 한 음 유지에서 숨이 먼저 과하게 새지 않는 쪽… | EFFORT | PASS |
| 소리가 너무 날카롭게 들려요 | EFFORT | 날카로움이 덜한 쪽으로, 작은~중간 강도에서 짧은 구절을 2~3회 부르세요. 음절 사이… | EFFORT | PASS |
| 거칠게 들려요 | CONTACT | 거친 인상이 덜한 쪽으로, 작은~중간 강도에서 짧은 구절을 2~3회 부르세요. 음절 사… | EFFORT | PASS |
...
| 고민 | Focus | 첫 처방 | Protocol | Status |
|---|---|---|---|---|
| 직접 입력 | EFFORT | 지금 궁금한 표현을 기준으로, 답답하게 느껴지는 한 구절을 편한 음량으로 2~3번 부르… | EFFORT | PASS |
...
- Focus 종류: CONTACT, DYNAMICS, EFFORT, REGISTER_CONNECTION, SAFETY, STABILITY
...
|---|---|---|---|
| DENSE_SOLID | EFFORT | — | EFFORT |
| BRIGHT_CLEAR | EFFORT | — | EFFORT |
| SOFT_SWEET | EFFORT | — | EFFORT |
| LIGHT_CLEAR | EFFORT | — | EFFORT |
| WARM_FULL | EFFORT | — | EFFORT |
| AIRY_DELICATE | EFFORT | — | EFFORT |
| INTENSE_DISTINCT | EFFORT | — | EFFORT |
| RECOMMEND_FOR_ME | EFFORT | — | EFFORT |
...
- Human name: `B`
- Intent: PUSHED
- Notes: 목잡이 — intentional high effort (contact_effort_controlled_v1)

| 항목 | Human | Analyzer | 결과 |
|---|---|---|---|
| effort | HIGH | MODERATE | PARTIAL_MATCH |
| register_connection | — | PARTIAL | NOT_LABELED |
| breathiness | LOW | LOW | MATCH |
| brightness | — | LOW | NOT_LABELED |
| presence | — | MID | NOT_LABELED |
